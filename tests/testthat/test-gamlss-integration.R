...
test_that("exact zero mass separates discrete, mixed and continuous families", {
  skip_if_not_installed("gamlss")
  skip_if_not_installed("gamlss.dist")
  skip_if_not_installed("distributions3")

  ## Mixed family (ZAGA): is_discrete() and is_continuous() are both FALSE,
  ## and the atom at zero is the fitted nu.
  set.seed(77)
  d <- data.frame(g = factor(rep(c("A", "B"), each = 40)))
  d$y <- gamlss.dist::rZAGA(nrow(d), mu = 2, sigma = 0.4,
                            nu = ifelse(d$g == "A", 0.35, 0.10))
  m <- gamlss::gamlss(y ~ g, nu.formula = ~ g, family = gamlss.dist::ZAGA,
                      data = d, trace = FALSE)
  nd <- data.frame(g = factor(c("A", "B"), levels = levels(d$g)))
  s <- gamlss_distribution_summary(m, nd, data = d)
  nu_hat <- as.numeric(stats::predict(m, what = "nu", newdata = nd,
                                      type = "response", data = d))
  expect_equal(s$prob_zero, nu_hat, tolerance = 1e-8)
  expect_true(all(s$prob_zero > 0))

  ## Continuous family on the whole real line (NO): there is no atom at zero,
  ## even though F(0) is large. This is the case a naive cdf(0) rule breaks.
  set.seed(78)
  d2 <- data.frame(g = factor(rep(c("A", "B"), each = 30)))
  d2$y <- gamlss.dist::rNO(nrow(d2), mu = 0.5, sigma = 1)
  m2 <- gamlss::gamlss(y ~ g, family = gamlss.dist::NO, data = d2, trace = FALSE)
  nd2 <- data.frame(g = factor(c("A", "B"), levels = levels(d2$g)))
  s2 <- gamlss_distribution_summary(m2, nd2, data = d2)
  expect_true(all(s2$prob_zero == 0))

  ## Continuous positive family (GA): also no atom.
  d3 <- make_ga_data(79, n = 20L)
  m3 <- gamlss::gamlss(y ~ trt, family = gamlss.dist::GA, data = d3, trace = FALSE)
  nd3 <- data.frame(trt = factor(levels(d3$trt), levels = levels(d3$trt)))
  expect_true(all(gamlss_distribution_summary(m3, nd3, data = d3)$prob_zero == 0))

  ## Discrete family (PO): the atom is the density at zero.
  set.seed(80)
  d4 <- data.frame(g = factor(rep(c("A", "B"), each = 30)))
  d4$y <- gamlss.dist::rPO(nrow(d4), mu = 2)
  m4 <- gamlss::gamlss(y ~ g, family = gamlss.dist::PO, data = d4, trace = FALSE)
  nd4 <- data.frame(g = factor(c("A", "B"), levels = levels(d4$g)))
  s4 <- gamlss_distribution_summary(m4, nd4, data = d4)
  mu4 <- as.numeric(stats::predict(m4, what = "mu", newdata = nd4,
                                   type = "response", data = d4))
  expect_equal(s4$prob_zero, exp(-mu4), tolerance = 1e-8)
})
